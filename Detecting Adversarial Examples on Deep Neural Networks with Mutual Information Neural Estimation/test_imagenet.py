...
def get_encoder(encoder_weights,fc_model_weights,vector_dimension):
    encoder = models.get_imagenet_encoder()
    fc_model = models.get_imagenet_full_connection_layers(vector_dimension)
    encoder.load_weights('networks/imagenet/autoencoder/'+encoder_weights+'.h5')
    fc_model.load_weights('networks/imagenet/autoencoder/'+fc_model_weights+'.h5')
...
def train_classifier(encoder_name,fc_model_name,savefile,x_train,y_train,x_test,y_test):
    ...
    classifier_d.save('networks/imagenet/autoencoder/'+savefile+'.h5')

def test_imagenet(encoder_name, fc_model_name, simple_classifier_name):
    vector_dimension = 64
    ...
    for i in range(len(model_names)):
        for j in range(len(adv_methods)):
            ...
            x_adv = np.load(
                'G:/adversarial_examples/imagenet10/' + model_names[i] + '/x_test_adv_' + adv_methods[j] + '.npy')
            ...
            positive_number = x.shape[0]
            negative_number = x_a.shape[0] * 2

            TP = positive_number - x_a_equal_num
            FN = x_a_equal_num
...
if __name__ == '__main__':
    ...
    for ablation in ablations:
        ...
        test_imagenet(encoder_name=encoder, fc_model_name=full_connection, simple_classifier_name=simple_classifier)
