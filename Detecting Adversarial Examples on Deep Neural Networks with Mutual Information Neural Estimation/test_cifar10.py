...
def get_classifier(dropout,vector_dimension):
    ...
    y = tf.keras.layers.Dense(1024, activation=None)(z)
...
def get_encoder(encoder_weights,fc_model_weights,vector_dimension):
    ...
    encoder = models.get_cifar10_encoder()
    fc_model = models.get_cifar10_full_connection_layers(vector_dimension)
    encoder.load_weights('networks/cifar10/autoencoder/' + encoder_weights + '.h5')
    fc_model.load_weights('networks/cifar10/autoencoder/' + fc_model_weights + '.h5')
...
def train_classifier(encoder_name,fc_model_name,savefile):
    ...
    classifier_d.fit(vectors,y_train,batch_size=256,epochs=100,validation_data=(x_vectors,y_test),callbacks=train_callbacks)
    # classifier_d.evaluate(x_adv_vectors, y_test, verbose=2)
    classifier_d.save('networks/cifar10/autoencoder/'+savefile+'.h5')

def test_cifar10(encoder_name, fc_model_name, simple_classifier_name,classifier,model_names):
    vector_dimension = 64
    ...
    classifier_d.load_weights('networks/cifar10/autoencoder/'+simple_classifier_name+'.h5')
    ...
    for i in range(len(model_names)):
        for j in range(len(adv_methods)):
            ...
            x_noise = np.add(x_test, np.random.normal(0, 0.01, x_test.shape))
            x_adv = np.load(
                'G:/adversarial_examples/cifar10/' + model_names[i] + '/x_test_adv_' + adv_methods[j] + '.npy')
            ...
            x_equal_num = x_equal_num
            x_n_equal_num = x_n_equal_num
            x_a_equal_num = x_a_equal_num
            ...
            print('recall:{0:0.3f}'.format(recall.numpy()), end=' ')
            print('precision:{0:0.3f}'.format(precision.numpy()), end=' ')
            print('F1:{0:0.3f}'.format(F1.numpy()), end=' ')
            print('specificity:{0:0.3f}'.format(specificity.numpy()), end=' ')
            print('accuracy:{0:0.3f}'.format(accuracy.numpy()))
...
if __name__ == '__main__':
    ...
    classifier = models.get_cifar10_local()
    ablations = ['local', 'global', 'prior', 'local_global', 'local_prior', 'global_prior']
    ...
    for ablation in ablations:
        ...
        test_cifar10(encoder_name=encoder, fc_model_name=full_connection, simple_classifier_name=simple_classifier,
                   classifier=classifier,model_names=model_names)
